...
class ShimadzuUVSLoader(BaseVendorLoader):
    ...
    def load_raw(self, path: Path) -> Any:
        try:
            ...
            raise LoaderReadError(f"Failed to read Shimadzu UVS file '{path}': {exc}") from exc
    ...
    def extract_metadata(self, raw_data: Any) -> Mapping[str, Any]:
        try:
            ...
            raise LoaderMetadataError(f"Failed to extract metadata from Shimadzu UVS: {exc}") from exc
    ...
    def to_universal(self, raw_data: Any, metadata: Mapping[str, Any]) -> Any:
        try:
            ...
            raise LoaderNormalizationError(f"Failed to normalize Shimadzu UVS data: {exc}") from exc
